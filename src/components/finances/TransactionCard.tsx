...
export const TransactionCard = memo(function TransactionCard({
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`w-full flex items-center gap-3 py-3 text-left border-b border-outline-variant/60 transition-colors active:bg-surface-container-low/50 ${
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {isNeed && !isExcluded && (
            <span
              className="inline-block h-1 w-1 rounded-full bg-muted-foreground/60 shrink-0"
              aria-label="Need"
            />
          )}
          {txn.details && (
        ...
        <p className="text-[11px] text-muted-foreground/80 truncate mt-0.5">
          ...
          <span className="text-muted-foreground/50"> · {formatDate(txn.date)}</span>
        </p>
      </div>
