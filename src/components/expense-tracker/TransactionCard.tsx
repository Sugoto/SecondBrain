...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <div
      className="relative text-[9px] px-1.5 py-0.5 h-4 shrink-0 rounded-md flex items-center gap-0.5 font-medium overflow-hidden"
      style={{
        background: `linear-gradient(135deg, ${color}30 0%, ${color}20 100%)`,
        color: color,
        boxShadow: `0 1px 4px ${color}20, inset 0 1px 0 rgba(255,255,255,0.2)`,
...
export function TransactionCard({
  ...
  return (
    <motion.button
      ...
      whileHover={{ 
        y: -2, 
        boxShadow: "0 8px 24px -4px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(59, 130, 246, 0.1)" 
      }}
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
